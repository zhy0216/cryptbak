...
fn saveMetadata(allocator: Allocator, metadata: BackupMetadata, output_dir: []const u8, key: [32]u8) !void {
    ...
    crypto.stream.chacha.ChaCha20IETF.xor(encrypted_buffer, buffer.items, 0, // counter
        enc_key, nonce);
...
fn loadMetadata(allocator: Allocator, output_dir: []const u8, key: [32]u8) !BackupMetadata {
    ...
    crypto.stream.chacha.ChaCha20IETF.xor(decrypted_buffer, encrypted_buffer, 0, // counter
        dec_key, nonce);
